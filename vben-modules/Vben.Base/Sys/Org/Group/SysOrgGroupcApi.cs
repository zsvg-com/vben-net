...
[Route("sys/org/groupc")]
[ApiDescriptionSettings("Sys", Tag = "组织架构-群组分类")]
public class SysOrgGroupcApi(SysOrgGroupCateService service) : ControllerBase
{
    /// <summary>
    /// 获取分类treeTable数据
    /// </summary>
    ...
    /// </summary>
    [HttpGet("info/{id}")]
    public async Task<SysOrgGroupCate> GetInfo(string id)
    {
    ...
    /// </summary>
    [HttpPost]
    public async Task<string> Post([FromBody] SysOrgGroupCate cate)
    {
        int count = await service.repo.Context.Queryable<SysOrgGroupCate>().Where(t => t.pid == cate.pid).CountAsync();
        cate.ornum = ++count;
    ...
    /// </summary>
    [HttpPut]
    public async Task<string> Put([FromBody] SysOrgGroupCate cate)
    {
    ...
    public async Task Delete(string ids)
    {
        ...
        {
            var cateCount = await
                service.repo.Context.Queryable<SysOrgGroupCate>().Where(it => it.pid == id).CountAsync();
            if (cateCount > 0)
            {
            ...
            var mainCount = await
                service.repo.Context.Queryable<SysOrgGroup>().Where(it => it.catid == id).CountAsync();
