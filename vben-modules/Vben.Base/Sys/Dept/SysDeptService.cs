...
[Service]
public class SysDeptService : BaseStrMainService<SysDept>
{
    public SysDeptService(SqlSugarRepository<SysDept> repo)
    {
        base.repo = repo;
    }

    public new async Task InsertAsync(SysDept dept)
    {
        //处理上级部门与部门层级信息
        dept.id = YitIdHelper.NextId() + "";
        if (dept.pid != null)
        {
            var parentTier = await repo.Context.Queryable<SysDept>()
    ...
    public new async Task UpdateAsync(SysDept dept)
    {
        //处理上级部门与部门层级信息
        if (dept.pid != null)
        {
        ...
        var olderTier = await repo.Context.Queryable<SysDept>()
            .Where(it => it.id == dept.id).Select(it => it.tier).SingleAsync();
        //事务包裹处理数据库操作
        await repo.Context.Ado.BeginTranAsync();
        await base.UpdateAsync(dept);
        //修改sys_dept时同时修改sys_org表name字段
        ...
        //修改sys_dept层级时需要同时修改子部门，部门下的员工，岗位的层级信息
    ...
    private async Task DealDeptTier(string oldTier, string newTier, string id)
    {
        var idNameList = await
            repo.Context.Ado.SqlQueryAsync<SidName>(
                "select id,tier as name from sys_dept where tier like @oldTier and id<>@id",
        ...
        await repo.Context.Updateable(dtList).AS("sys_dept").WhereColumns("id").ExecuteCommandAsync();
    }
    ...
    private async Task DealUserTier(string oldTier, string newTier)
    {
        var idNameList = await
            repo.Context.Ado.SqlQueryAsync<SidName>(
                "select id,tier as name from sys_user where tier like @oldTier",
                new { oldTier = oldTier + "%" });
        ...
        await repo.Context.Updateable(list).AS("sys_user").WhereColumns("id").ExecuteCommandAsync();
    }
    ...
    private async Task DealPostTier(string oldTier, string newTier)
    {
        var idNameList = await
            repo.Context.Ado.SqlQueryAsync<SidName>(
                "select id,tier as name from sys_post where tier like @oldTier",
                new { oldTier = oldTier + "%" });
        ...
        await repo.Context.Updateable(list).AS("sys_post").WhereColumns("id").ExecuteCommandAsync();
    }
    ...
    public async Task DeleteAsync(string[] ids)
    {
        ...
        await repo.Context.Deleteable<SysDept>().In(ids).ExecuteCommandAsync();
        await repo.Context.Ado.CommitTranAsync();
    }

    private async Task<bool> CanDelete(string id)
    {
        var tier = await repo.Context.Queryable<SysDept>()
            .Where(it => it.id == id).Select(it => it.tier).SingleAsync();

        var deptCount = await
            repo.Context.Queryable<SysDept>().Where(it => it.tier.StartsWith(tier)).CountAsync();
        if (deptCount > 1)
        {
            return false;
        }
        ...
        var userCount = await
            repo.Context.Queryable<SysUser>().Where(it => it.tier.StartsWith(tier)).CountAsync();
        if (userCount > 0)
        {
            return false;
        }
        ...
        var postCount = await
            repo.Context.Queryable<SysPost>().Where(it => it.tier.StartsWith(tier)).CountAsync();
        if (postCount > 0)
        {
            return false;
        }
    ...
    private async Task UpdateOrnumAsync(SysDept dept)
    {
        await repo.Context.Updateable<SysDept>()
    ...
    public async Task PostMove(TreeMovePo po)
    {
        ...
        List<SysDept> list2 = await repo.Context.Queryable<SysDept>()
        ...
        if (po.type == "inner")
        {
            ...
            int count = await repo.Context.Queryable<SysDept>().Where(t => t.pid == po.droid).CountAsync();
        ...
        else if (po.type == "before")
        {
            ...
            List<SysDept> list = await repo.Context.Queryable<SysDept>()
        ...
        {
            var dropDept = await SingleAsync(po.droid);
            int count = await repo.Context.Queryable<SysDept>().Where(t => t.pid == dropDept.pid)
                .CountAsync();
